...
from core.keyboard import start_inline, start_button, new_film
...
async def start(message: Message):
    ...
    if message.chat.id == ADMIN_ID:
        await message.reply("Привет, хозяин", reply_markup=ADMIN_ID)
...
<a href='{title_info["url"]}'>___ Ссылка на фильм ___</a>"""
    ...
    if message.text.lower() == "новинка":
        ...
        await message.answer("Вы в меню", reply_markup = start_button)

    elif message.text.lower() == "рандомный фильм":
        await message.reply(random.choice(new_film_kino))
    
    elif message.text.lower() == "все новинки фильма":
        count = 0
...
async def send_random_value(call: CallbackQuery):
    ...
    rezka_parser()
    await call.message.answer("Подождите несколько секунд, обновляем </b>json file</b>")
